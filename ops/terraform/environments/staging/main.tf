terraform {
  backend "gcs" {
    bucket = "mrt-datacleaner-s-sbu-terraform-state"
    prefix = "terraform/state"
  }
}

locals {
  project_id = "${local.project_prefix}-9b8c"
  project_prefix = "mrt-datacleaner-s"
  app_location = "us-central"
  cloud_tasks_location = "us-central1"
}

provider "google" {
  project = "mrt-datacleaner-s-9b8c"
}

locals {
  frontend_url = "dct.staging.merits.com"
  merit_api_base_url = "https://sandbox-api.merits.com/v2"
